...
var curveDiscreateCheck = document.getElementById("discretizationCurves");
var curveDiscreateCount = document.getElementById("discretizationCurvesCount");
...
window.onload = function() {
  ...
  curveDiscreateCount.disabled = !curveDiscreateCheck.checked;
};
...
curveDiscreateCheck.onchange = function() {
  curveDiscreateCount.disabled = !curveDiscreateCheck.checked;
};
...
var convert = function(font) {
  const discretizationCount = curveDiscreateCount.value;
    ...
    if (glyph.unicode !== undefined) {
      ...
      if (needToExport) {
        ...
        glyph.path.commands.forEach(function(command, i) {
          ...
          if (command.type.toLowerCase() === "q" && curveDiscreateCheck) {
            const { x, y } = lastCoordinate;
            lastCoordinate.x = command.x;
            lastCoordinate.y = command.y;

            const { x: endX, y: endY, x1, y1 } = command;
            const curve = new Bezier(x, y, x1, y1, endX, endY);

            console.log("output curve be flatten to ", discretizationCount);
            const LUT = curve.getLUT(Number(discretizationCount) || 2);

            // change curve into lines binded together
            token.o += LUT.map(
              ({ x, y }) =>
